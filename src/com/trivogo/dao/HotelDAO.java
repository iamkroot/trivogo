...
public class HotelDAO {
        ...
        try {
            ...
            while (rs.next()) {
                hotel = new Hotel(rs.getInt("id"), rs.getString("name"), rs.getString("location"),
                        ...
                        rs.getString("stdRoomAmenities"), rs.getFloat("stdRoomRate"), (int) rs.getFloat("rating"));
            }
    ...
    public static List<Hotel> getHotelsByLocation(String location) {
        ...
        try {
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                Hotel hotel = new Hotel(rs.getInt("id"), rs.getString("name"), rs.getString("location"), rs.getInt("numDexRooms"), rs.getInt("numStdRooms"), rs.getString("dexRoomAmenities"), rs.getInt("dexRoomRate"), rs.getString("stdRoomAmenities"), rs.getInt("stdRoomRate"), (int) rs.getFloat("rating"));
                hotels.add(hotel);
            }
            ps.close();
        } catch (SQLException e) {
